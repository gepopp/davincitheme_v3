<template>
  ...
  <div class="relative overflow-hidden min-h-halfscreen">
    <div class="md:hidden">
      <div class="flex justify-between">
        <div class="flex bg-golden text-white p-3 uppercase z-40 cursor-pointer " @click="showFilter = !showFilter">
          <span>filtern</span>
          <svg class="w-6 h-6 text-white" v-show="!showFilter" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="flex bg-golden text-white p-3 uppercase z-40 cursor-pointer " @click="showContact = !showContact">
          <svg class="w-6 h-6 text-white" v-show="!showContact" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 13l-7 7-7-7m14-8l-7 7-7-7"></path>
          </svg>
          <svg class="w-6 h-6 text-white" v-show="showContact" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 11l7-7 7 7M5 19l7-7 7 7"></path>
    ...
    <div class="grid grid-cols-4 gap-5">
      <div class="hidden md:grid border-r border-golden my-5">
        <table-filters
            :min-price="minPrice"
            :max-price="maxPrice"
            :min-rent="minRent"
      ...
      <div class="col-span-4 md:col-span-3 pr-0 md:pr-5">
        <div class="w-full max-w-full grid grid-cols-4 md:grid-cols-8 gap-x-3 border-b border-golden">
          <div class="uppercase text-xs font-semibold p-2 my-3 flex flex-col items-center justify-center cursor-pointer" @click="sort('top')">
            <div class="flex flex-col md:flex-row items-center space-x-1">
              ...
              <div class="flex flex-col mt-auto" :class="sorted == 'top' ? 'bg-darkblue text-white' : 'text-darkblue'">
                <svg class="w-4 h-4" v-show="operator == '<'" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <transition name="fade" v-for="top in filteredTops" :key="top.id">
          <div class="w-full max-w-full grid grid-cols-4 md:grid-cols-8 gap-x-3 border-b border-golden bg-clip-content" :class="top.sold || !top.status ? 'bg-gray-200' : 'hover:bg-gray-100'">
...
export default {
  ...
  mounted() {
    ...
    this.$root.$on('tableFilter', (data) => {
      ...
      this.wfl = data.wfl;
      this.filter();
      if(data.close) this.showFilter = false
    });
  ...
  methods: {
    ...
    filter() {
      ...
      this.filteredTops = this.tops.filter((top) => {
        ...
        return (
                top.verkaufspreis >= this.price[0]
                && top.verkaufspreis <= this.price[1])
            && (top.zimmer >= this.rooms[0]
                && top.zimmer <= this.rooms[1])
            && (top.wfl >= this.wfl[0] && top.wfl <= this.wfl[1]);
