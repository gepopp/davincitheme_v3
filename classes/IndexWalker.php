...
class IndexWalker {
	...
	protected function setupPostColumns() {
		...
		$this->addBlogPosts( array_sum( $this->postColumns ) % 4 );
	...
	public function addBlogPosts( $post_count ) {

		$query = new \WP_Query( [ 'posts_per_page' => $post_count ] );
    ...
    public function mergedQuery(){
	    ...
	    $all = new \WP_Query([
                'post_type' => ['project', 'frontpage', 'posts'],
                'post__in' => array_merge($ids, $this->blogPosts)
        ]);
